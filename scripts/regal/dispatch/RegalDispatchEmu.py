...
from RegalDispatchShared import dispatchSourceTemplate, origHeaderTemplate

from Emu       import emuFindEntry, emuCodeGen, emuGetOriginateList

##############################################################################################

def apiEmuOriginateCode( e, apis ):
  code = ''

  o = emuGetOriginateList( e['formulae'], apis )
  if len(o):
    code += 'struct EmuOriginateProc%s {\n' % e['suffix']
    code += '\n'
    code += '  EmuOriginateProc%s() {\n' % e['suffix']
...
#include "RegalFilt.h"'''
  ...
  for e in emu:
    ...
    s['HEADER_NAME']   = 'REGAL_ORIG_%s_H' % e['suffix'].upper()
    s['LOCAL_CODE']      =  apiEmuOriginateCode( e, apis )
    s['LOCAL_INCLUDE']   = ''
    ...
    outputCode( '%s/RegalOrig%s.h' % (args.srcdir, e['suffix']), origHeaderTemplate.substitute(s))
